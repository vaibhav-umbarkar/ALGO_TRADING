...
def CRYPTO_BOT():
    try:
        ...
        for symbol in watchlist:
            ...
            log_status = get_status(log_data, symbol) # Get Log Status
...
def RISK_MANAGEMENT():
    try:
        ...
        for data in log_data.get('data', []):
            ...
            org_price = data['org_price'] # Get Original Purchasing Price of Crypto
            temp_price = data['temp_price'] # Get Temporary Purchasing Price of Crypto
            crypto_exists = find_coin(symbol)  # Check the coin exists in portfolio or not
            
            stop_loss_price = get_stop_loss(temp_price, 10) # Calculate Stop Loss Price (Note: 10%)
            target_price_1 = get_target(temp_price, 5) # Calculate Dynamic Target Price (Note: 5%)
            target_price_2 = get_target(org_price, 50) # Calculate Long Term Target Price (Note: 50%)
            
            # Auto Set Log Status
            auto_status_change(file_path, log_data, crypto_exists, symbol, current_price)
            
            # For Stop Loss Hit (10%)
            if current_price <= stop_loss_price and crypto_exists == True:
                sell_order(symbol) # Place Sell Order
                update_log_file(file_path, symbol, 0, 0, 'manual') # Update Log Status
            ...
            # For Dynamic Target (5%)
            elif current_price >= target_price_1 and crypto_exists == True:
                update_log_file(file_path, symbol, org_price, current_price, 'auto') # Update Log Buy Price for Dynamic Stop Loss
                logging.info(f'Dynamic Target Set for {symbol}')    
            
            # For Target Hit (50%)
            elif current_price >= target_price_2 and crypto_exists == True:
                update_log_file(file_path, symbol, org_price, temp_price, 'hold') # Update Log Status
